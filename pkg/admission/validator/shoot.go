...
import (
	...
	hcloudvalidation "github.com/23technologies/gardener-extension-provider-hcloud/pkg/apis/hcloud/validation"
...
func (s *shoot) Validate(ctx context.Context, newObj, oldObj client.Object) error {
	...
	var credentials *hcloud.Credentials
...
func (s *shoot) validateShootCreation(shoot *core.Shoot, credentials *hcloud.Credentials, cloudProfileSpec *gardencorev1beta1.CloudProfileSpec) error {
...
func (s *shoot) validateShootUpdate(oldShoot, shoot *core.Shoot, credentials *hcloud.Credentials, cloudProfileSpec *gardencorev1beta1.CloudProfileSpec) error {
	...
	if errList := hcloudvalidation.ValidateWorkersUpdate(oldValContext.shoot.Spec.Provider.Workers, valContext.shoot.Spec.Provider.Workers, workersPath); len(errList) > 0 {
...
func (s *shoot) validateShoot(context *validationContext) field.ErrorList {
	...
	if context.shoot.Spec.Networking != nil {
		allErrs = append(allErrs, hcloudvalidation.ValidateNetworking(context.shoot.Spec.Networking, nwPath)...)
	}
	allErrs = append(allErrs, hcloudvalidation.ValidateWorkers(context.shoot.Spec.Provider.Workers, context.cloudProfileConfig, workersPath)...)
	return allErrs
}
...
func newValidationContext(decoder runtime.Decoder, shoot *core.Shoot, cloudProfileSpec *gardencorev1beta1.CloudProfileSpec) (*validationContext, error) {
	if shoot.Spec.Provider.InfrastructureConfig == nil {
		return nil, field.Required(infraConfigPath, "infrastructureConfig must be set for hcloud shoots")
	}
	...
	if shoot.Spec.Provider.ControlPlaneConfig == nil {
		return nil, field.Required(cpConfigPath, "controlPlaneConfig must be set for hcloud shoots")
	}
